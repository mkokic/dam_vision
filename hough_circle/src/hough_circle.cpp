...
// Constructor
    ...
    void Hough::houghDetection(const cv::Mat& src_blur, const cv::Mat& src_display, int counter)
    {
        ...
        if (counter > 25)
        {
            ...
            for(size_t i = 0; i < contours.size(); i++)
            {
                ...
                if(iLastX == -1 && (area < 1000 || area > 3000))
                {
                    continue;
                }
                if(area > 1000 && area < 3000)
                {
                    ...
                    if (sqrt((box.center.x - iLastX) * (box.center.x - iLastX) + (box.center.y - iLastY) * (box.center.y - iLastY)) < 5.0)
                    {
                        iLastX = box.center.x;
                        iLastY = box.center.y;
                ...
                //     }
                //     if (sqrt((box.center.x - iLastX) * (box.center.x - iLastX) + (box.center.y - iLastY) * (box.center.y - iLastY)) < 25.0)
                //     {
                //         drawContours( display, contours, i, cv::Scalar(0,0,255), 2, 8, hierarchy, 0, cv::Point() );
                //         iLastX = box.center.x;
                //         iLastY = box.center.y;
    ...
    {
        h_lowInit=config.h_low;
        h_highInit= config.h_high;
        s_lowInit=config.s_low;
        s_highInit= config.s_high;
        v_lowInit=config.v_low;
        v_highInit= config.v_high;
    ...
    void Hough::run(){
        if (do_circle_detection)
        {
           if(!src.empty())
            ...
            cv::Mat hue_image;
            cv::inRange(hsv, cv::Scalar(h_lowInit, s_lowInit, v_lowInit), cv::Scalar(h_highInit, s_highInit, v_highInit), hue_image);
            cv::GaussianBlur(hue_image, hue_image, cv::Size(9, 9), 10, 10);
            cv::threshold(hue_image, hue_image, 50, 255, cv::THRESH_BINARY);
            ...
            houghDetection(hue_image, src, ct_);
